...
public class SeasonalityDAO {
    ...
    /**
     * Saves an import and its thresholds as one unit. A failed insert cannot
     * leave a user with deleted history or thresholds.
     */
    public static int saveImportedMonthlyData(int userId, List<MonthlySeasonData> dataList,
                                              SeasonThreshold threshold, boolean replaceExisting) {
        String insertMonthly = "INSERT INTO MonthlySeasonData (UserID, MonthYear, MonthName, " +
                "OccupancyRate, TotalRevenue, AvgRoomRate, ClassifiedSeason) VALUES (?, ?, ?, ?, ?, ?, ?)";
        Connection conn = null;
        try {
            conn = DBConnection.getConnection();
            conn.setAutoCommit(false);
            if (replaceExisting) {
                try (PreparedStatement deleteMonthly = conn.prepareStatement(
                        "DELETE FROM MonthlySeasonData WHERE UserID = ?");
                     PreparedStatement deleteThreshold = conn.prepareStatement(
                             "DELETE FROM SeasonThreshold WHERE UserID = ?")) {
                    deleteMonthly.setInt(1, userId);
                    deleteMonthly.executeUpdate();
                    deleteThreshold.setInt(1, userId);
                    deleteThreshold.executeUpdate();
                }
            }

            try (PreparedStatement removeThreshold = conn.prepareStatement(
                    "DELETE FROM SeasonThreshold WHERE UserID = ?");
                 PreparedStatement insertThreshold = conn.prepareStatement(
                         "INSERT INTO SeasonThreshold (UserID, ThresholdLowNormal, ThresholdNormalPeak, " +
                                 "ThresholdPeakSuperPeak, IsAutoGenerated) VALUES (?, ?, ?, ?, ?)");
                 PreparedStatement insert = conn.prepareStatement(insertMonthly)) {
                // There is one active threshold set per user; replacing it keeps import behavior deterministic.
                removeThreshold.setInt(1, userId);
                removeThreshold.executeUpdate();
                insertThreshold.setInt(1, userId);
                insertThreshold.setDouble(2, threshold.getThresholdLowNormal());
                insertThreshold.setDouble(3, threshold.getThresholdNormalPeak());
                insertThreshold.setDouble(4, threshold.getThresholdPeakSuperPeak());
                insertThreshold.setBoolean(5, threshold.isAutoGenerated());
                insertThreshold.executeUpdate();

                for (MonthlySeasonData data : dataList) {
                    insert.setInt(1, userId);
                    insert.setString(2, data.getMonthYear());
                    insert.setString(3, data.getMonthName());
                    insert.setDouble(4, data.getOccupancyRate());
                    insert.setDouble(5, data.getTotalRevenue());
                    insert.setDouble(6, data.getAvgRoomRate());
                    insert.setString(7, data.getClassifiedSeason());
                    insert.addBatch();
                }
                int saved = 0;
            ...
            if (conn != null) {
                try {
                    ...
                    logger.warn("Rollback failed while saving monthly import for user {}", userId, rollbackError);
                }
            }
            throw new IllegalStateException("Unable to save monthly import", e);
        } finally {
            if (conn != null) {
                try {
                    conn.close();
                } catch (SQLException closeError) {
                    logger.warn("Failed to close monthly import connection for user {}", userId, closeError);
                }
            }
        }
    }
     ...
     */
    public static boolean deleteMonthlyDataById(int dataId, int userId) {
        String sql = "DELETE FROM MonthlySeasonData WHERE DataID = ? AND UserID = ?";
     ...
     */
    public static boolean updateMonthSeason(int dataId, int userId, String newSeason) {
